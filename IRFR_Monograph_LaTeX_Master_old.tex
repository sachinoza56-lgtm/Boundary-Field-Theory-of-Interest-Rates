...
the forward curve becomes a stochastic field over remaining maturity τ.
Calendar time evolves the field, while the maturity coordinate rolls
deterministically toward the short end. The point τ=0 is therefore not
...
The second part of the statement concerns the boundary at τ=0. The
...
selection is made by the boundary law. A Neumann/free boundary, a fixed
...
Here t denotes calendar time and τ denotes remaining maturity. For a
...
The short end, τ=0, has a special role. In rolling-maturity coordinates
...
expression. Monetary policy is represented as a boundary law at τ=0.
...
Markov in the ordinary short-rate sense. It is Markovian as a
boundary-value evolution of the IRFR field. The recovery of Markovian
structure occurs because the state geometry is changed.
...
where t is calendar time and τ is time to maturity. The forward curve is
therefore represented as a stochastic field over the half-line {[}0,∞).

This chapter explains why this change of primitive matters. The move
from (t,T) to (t,τ) does not merely relabel the standard forward-rate
...
The point τ=0 becomes a true boundary of the maturity domain. The model
...
x(t,·) is a curve over the maturity domain, while t ↦ x(t,·) describes
...
at time t=0. At that date it sits at τ=5. One year later, if the
calendar maturity date is unchanged, the same contract has τ=4. It has
...
calendar-time evolution: ∂ₜx(t,τ)

maturity transport: −∂\_τx(t,τ)
...
Then ∂τx(t,τ)=b, so the deterministic roll-down contribution is
...
important. Since fixed maturities move in the direction of decreasing τ,
all rolling-maturity trajectories are transported toward the boundary
τ=0. The boundary is therefore not appended to the model after the fact;
...
\[
x : {[}0,\infty) × {[}0,\infty) \to ℝ,
\]

where t indexes calendar time and τ indexes remaining maturity. For each
t, x(t,τ) is a function of τ. For each τ, x(t,τ) evolves stochastically
...
object whose values across τ are linked by the rolling structure of the
...
represented as x(t,τ), where τ is the remaining maturity. This makes it
...
maturity τ. This is not merely a visual or notational symmetry. It is
...
The point τ=0 corresponds to the instantaneous short end. Every
...
calendar time carries the field from larger τ toward smaller τ. The
boundary at τ=0 is therefore not an arbitrary endpoint. It is the
location at which rolling maturities arrive.

This has two consequences.

First, the drift of the IRFR field must include a transport correction.
If the equilibrium curve depends on τ, then a point rolling down the
...
Second, the boundary at τ=0 requires a closure condition. The field
...
The boundary at τ=0 is not only mathematically natural. It is also
...
The important point is simply that τ=0 is the mathematically natural
...
enters as a boundary law at τ=0, and its effects propagate through the
...
Conversely, given x(t,τ), the conventional forward rate can be recovered
by
...
(t,τ) representation is natural for studying rolling maturity, field
...
In particular, the boundary at τ=0 is explicit in IRFR coordinates. The
...
First, remaining maturity is a transport coordinate: dτ/dt = −1.

Second, the forward curve is a field over the maturity domain: for each
t, τ ↦ x(t,τ) is the curve.
...
Fourth, the short end τ=0 is a boundary. It is mathematically required
...
A stochastic model for x(t,τ) must specify how the field evolves in
...
Finally, the model must be closed at τ=0. This is the role of the
...
τ=T−t turns the forward curve into a maturity-domain field, exposes
deterministic rolling transport, and identifies the short end τ=0 as a
boundary.
...
Boundary at τ=0 & Axiom 4: boundary closure \\
\end{longtable}
...
Without Axiom 2, the coordinate τ would lose its meaning as a
...
the interpretation of x(t,τ) as a rolling field rather than a collection
...
axiom determines how the system is closed at τ=0, and therefore where
...
For each remaining maturity τ\textgreater0, the local evolution of the
...
where W\_t is a Brownian motion, f is the IRFR drift-operator
...
At each remaining maturity τ, the local field increment
...
exposure at remaining maturity τ is sensitive. The full mark-to-market
...
field. The diffusion-scale term g describes local uncertainty, with √2g
...
τ, rather than on the calendar maturity date T itself. Equivalently,
...
structural dynamics to two contracts that both have τ=5, merely because
they are observed at different calendar dates. This would break the
interpretation of τ as a maturity-domain coordinate rather than a
calendar label.
...
Without it, the forward curve could still be written as x(t,τ), but the
...
The point τ=5 represents five years of remaining maturity, not a
...
τ, then that dependence is not merely a label attached to a particular
...
analysis. Once τ is the maturity-domain coordinate, τ=0 is the boundary
of that domain.
...
If b\textgreater0, the equilibrium curve is upward sloping in τ, and the
...
The maturity domain is the half-line {[}0,∞). The point τ=0 is the
...
For each remaining maturity τ, the local stochastic evolution is written
as
...
SDE notation, the volatility coefficient would be σ=√2g. This chapter
...
Axiom 1 states that, at each remaining maturity τ, the IRFR field
...
denote the probability density of the IRFR value x(t,τ). The Kolmogorov
...
depend on the maturity coordinate τ. The role of Axioms 2 and 3 is to
restrict that τ-dependence. The KFE is therefore the bridge between
...
For a fixed calendar maturity T, the remaining maturity is τ=T-t. The
...
respect to τ. This is the moment-transport requirement.
...
roll-down whenever m′(τ) is non-zero. The drift must therefore include a
...
roll-down contribution is -m′(τ), while the compensating term in the
fixed-τ rolling-field drift appears with the opposite sign. Since the
...
The term -m′(τ) inside f produces the +m′(τ) transport correction in the
...
Closure of the first moment requires E{[}f(x,t,τ){]} to depend only on
M₁(t,τ), rather than on higher moments of the density. This holds for
...
where A(τ) and B(τ) are functions of remaining maturity. Then

\[
E{[}f(x,t,\tau){]} = A(\tau)M_1(t,\tau)+B(\tau)
\]

At equilibrium, M₁(t,τ)=m(τ). The moment-transport condition requires
the actual expected drift, -E{[}f{]}, to match the required transport of
the equilibrium curve. This imposes a relation between A(τ), B(τ), and
m(τ).

Choosing the displacement scale as a², the affine drift can be written
as
...
the standard mean-reverting displacement term together with the fixed-τ
...
drift -f, it produces the mean-reverting contribution -a²{[}x-m(τ){]}.
...
produces the +m′(τ) transport correction required by fixed-τ
...
The transport component is constant. It vanishes only when β=0, that is,
...
a²{[}x-m(τ){]} inside f produces the standard mean-reverting
contribution -a²{[}x-m(τ){]} in the SDE.
...
so that the conventional volatility coefficient is √2g(τ)=√2aH(τ), then
the instantaneous covariance between maturities τ₁ and τ₂ is

\[
\operatorname{Cov}(dx(t,\tau_1), dx(t,\tau_2)) = 2a^2H(\tau_1)H(\tau_2)dt
\]

The normalisation with √2 in the SDE is chosen for later algebraic
convenience. The economically relevant object is the maturity loading
H(τ), which determines how the common stochastic shock is expressed
across the curve.

This covariance kernel is rank one. The stochastic dimension is one, but
the deterministic maturity loading H(τ) can be non-trivial.
...
variance coefficients. In practice, this means equating the τ-dependent
...
component inside f, generating the required +m′(τ) correction in the
actual drift.
...
together with the fixed-τ transport correction.
...
\[
coefficient matching restricts the maturity loading;
\]
\[
the resulting maturity equation generates an r-branch;
\]
...
loading H(τ) may have non-trivial shape.
...
In practice, this means that the τ-dependent coefficients generated by
...
H(τ). In canonical form, this equation may be written as

\[
H″(\tau) + \lambda H′(\tau) + \mu H(\tau) = 0,
\]

where λ and μ are constants determined by the drift-diffusion scale and
...
k₁ \textgreater{} 0, k₂ \textgreater{} 0.

The constants C₁ and C₂ are not determined by the interior equation
...
\[
the interior law determines the admissible modes;
\]
\[
the boundary law selects their realised combination
\]
...
\[
interior admissibility produces the repeated-root mode;
\]
\[
boundary closure selects its economic realisation
\]
...
\[
humped volatility as boundary-selected admissible mode;
\]
rather than

\[
humped volatility as exogenous volatility input
\]
...
selection requires Axiom 4: boundary closure at τ = 0.
...
\[
Axioms 1-3 determine admissible modes;
\]
\[
Axiom 4 selects the realised boundary regime
\]
...
boundary. But they do not, by themselves, determine what happens at τ =
0.
...
\[
Which maturity loadings are dynamically allowed?
\]
...
\]
The constants C₁ and C₂ are not fixed by the interior equation alone.
...
\]
\[
Dirichlet fixed-boundary closure;
\]
\[
Robin partial-anchoring closure
\]
...
Dirichlet & H(0)=0 & pinned short end & suppresses C₁ in the
repeated-root case \\
Robin & αH(0)+βH′(0)=γ & partial anchoring & mixes admissible
components \\
\end{longtable}

Neumann/free closure represents the benchmark in which no explicit
...
The label ``Neumann/free'' is used here in the broad boundary-value sense of a non-pinned natural endpoint regime. The operative loading benchmark is the retained exponential mode, not the illustrative condition $H'(0)=0$.
...
H_N(0) = A_N
\]
Thus, unless A\_N=0, the short end retains stochastic loading. The
...
form. A full state-space formulation could express natural closure
...
τ=0. The short end remains part of the stochastic field rather than
...
\[
x(t,0)=prescribed boundary value
\]
...
\[
r=1 repeated-root admissibility
\]
\[
Dirichlet short-end boundary closure
\]
...
\]
where α, β, and γ encode the boundary regime.
...
H′(0)=C_2-kC_1
\]
A Robin condition therefore imposes a linear relation between C₁ and C₂.
...
For r≠1, the admissible loading is a combination of distinct exponential
...
\[
interior admissibility creates the mode space;
\]
\[
boundary closure selects the realised mode
\]
...
imposing a fixed short-end value. In this sense, natural closure
represents the non-pinned benchmark against which the Dirichlet and
Robin regimes are compared.

natural closure leaves the short end stochastic;
...
the model imposes no additional policy-pinning condition at τ=0. The
...
The short end τ=0 is economically distinguished, but natural closure
...
coordinate τ. The analogous maturity-domain idea is that the field meets
...
natural closure is a compatibility condition, not a pinned-value
...
whenever A\_N≠0. It therefore represents a regime in which short-end
...
This makes the economic distinction transparent. Under natural closure,
...
\bottomrule\noalign{}
\endlastfoot
Neumann/free & A\_N e\^{}\{-a\_N²τ\} & retained & non-pinned
benchmark \\
Dirichlet & C\_D τ e\^{}\{-a\_D²τ\} & suppressed & pinned boundary \\
\end{longtable}
...
natural closure: free short end;
...
H\_D\^{}\{(1)\}(τ)=C\_Dτe\^{}\{-a\_D²τ\},

which suppresses short-end loading.

natural closure closes the field without pinning the short end.
...
The endpoint τ=0 is the instantaneous short end. A Dirichlet condition
...
where x\_0(t) is a prescribed short-end boundary value. In the most
idealised fixed-policy case, x\_0(t) may be constant over the relevant
...
h\textquotesingle(τ)=e\^{}\{-a²τ\}(1-a²τ).
...
The location of the maximum is controlled by a². A larger a² shifts the
hump toward shorter maturities. A smaller a² shifts it toward longer
maturities. Thus a² controls not only the strength of local mean
...
is non-negative on the half-line whenever C\_D≥0. It satisfies
...
For each fixed maturity τ\textgreater0, the shifted Dirichlet dynamics
...
x\textgreater x\_min. The same Feller boundary classification as in the
positive-state benchmark shows that the lower boundary y=0 is
inaccessible for each fixed τ\textgreater0, provided the process starts
...
fixed-τ positivity to the full maturity-field statement
...
X\_\{t+s\} is determined by X\_t, the interior operator, and the
...
policy enters locally at τ=0;

the admissible field transmits the effect across τ\textgreater0.
...
Since H(0)=C\_1, the boundary condition gives C\_1=0. The selected
...
stationary density for fixed maturities τ\textgreater0, and the lower
...
H\_R\textquotesingle(τ)={[}C\_1-a\_R²(C\_0+C\_1τ){]}e\^{}\{-a\_R²τ\},

so

H\_R\textquotesingle(0)=C\_1-a\_R²C\_0.

A homogeneous Robin condition may be written as

H\_R\textquotesingle(0)=-ρH\_R(0),

where

ρ=-H\_R\textquotesingle(0)/H\_R(0)

whenever H\_R(0)≠0. Substituting the boundary value and derivative gives
...
H\_R(τ)=C\_0{[}1+(a\_R²-ρ)τ{]}e\^{}\{-a\_R²τ\}.

Writing k=a\_R², this is

H\_R(τ)=C\_0{[}1+(k-ρ)τ{]}e\^{}\{-kτ\}.

The parameter ρ measures the negative boundary slope of the loading
...
If C\_1=0, the loading is purely exponential:
...
If C\_0=0, the loading becomes

H\_R(τ)=C\_1τe\^{}\{-a\_R²τ\}.
...
and pinned-boundary suppression by varying the relative weights of C\_0
and C\_1.
...
Neumann/free & A\_N e\^{}\{-a\_N²τ\} & retained & non-pinned
benchmark \\
Robin & (C\_0+C\_1τ)e\^{}\{-a\_R²τ\} & partially retained & partial
anchoring \\
Dirichlet & C\_D τe\^{}\{-a\_D²τ\} & suppressed & pinned boundary \\
\end{longtable}
...
H\_R(0)=C\_0, H\_R′(0)=C\_1-a\_R²C\_0.
...
The parameter ρ controls how the loading leaves the boundary. If
ρ=a\_R², then C\_1=0, and the loading is pure exponential. If
ρ\textless a\_R², then C\_1\textgreater0, and the loading contains a
positive boundary-induced linear-exponential component. If
ρ\textgreater a\_R², then C\_1\textless0, and the loading may eventually
change sign.
...
H\_R(τ)=C\_0(1+qτ)e\^{}\{-kτ\}.

The derivative is

H\_R\textquotesingle(τ)=C\_0{[}(q-k)-kqτ{]}e\^{}\{-kτ\}.
...
The shape depends on the anchoring parameter. If ρ=a\_R², then q=0 and
the loading is pure exponential. If ρ\textless a\_R², then
q\textgreater0 and the loading remains globally non-negative for
C\_0\textgreater0. If ρ\textgreater a\_R², then q\textless0 and the
...
is non-negative on the half-line for C\_D≥0 and vanishes at the
...
density for each fixed τ\textgreater0 and that the lower boundary is
...
preserve sign. If C\_0\textgreater0, the necessary and sufficient
...
If ρ\textgreater k, the linear term eventually becomes negative, and the
...
value H\_R(0)=C\_0 represents the amount of short-end loading retained.
The derivative H\_R′(0) represents how the maturity loading leaves the
...
The anchoring parameter ρ therefore acts as a transmission coefficient.
...
Neumann/free → short-end stochastic loading retained;

\[
Dirichlet \to short-end stochastic loading suppressed;
\]

Robin → short-end stochastic loading partially retained.
...
ρ, changes the realised maturity loading without changing the interior
admissible equation.
...
In empirical implementation, ρ becomes an estimable parameter linking
...
This loading retains short-end stochastic loading unless C\_0=0. It
therefore represents partial policy anchoring.

For ρ≤a\_R², the selected loading can preserve a non-negative
lower-boundary interpretation on the half-line, assuming
C\_0\textgreater0. Under the shifted positive-state KFE structure, the
Robin case then inherits the fixed-maturity inverse-gamma and Feller
logic described for Dirichlet in Appendix B. For ρ\textgreater a\_R²,
...
\[
the interior law determines the admissible mode space;
\]
\[
the boundary law selects the realised maturity loading
\]
...
When g(τ) = aH(τ), the instantaneous covariance between maturity points
is
...
Thus H(τ) is the deterministic maturity profile through which the one
stochastic factor is transmitted across the curve.

The purpose of this chapter is to interpret the boundary-selected forms
of H(τ) as observable volatility loadings.

\[
observable volatility shape is not imposed directly;
\]
...
conventional SDE volatility coefficient is √2g.
...
The economically relevant maturity shape is H(τ). The constants a and √2
set scale and normalisation. The function H(τ) determines where along
...
the deterministic maturity loading can vary across τ.
...
is proportional to H(τ). Thus, within the one-factor restriction, the
...
When C₀≠0, this can also be written in anchoring-ratio form as
...
\bottomrule\noalign{}
\endlastfoot
Neumann/free & A\_N exp(-a\_N²τ) & retained & non-pinned benchmark \\
Dirichlet & C\_D τ exp(-a\_D²τ) & suppressed & pinned boundary \\
Robin & (C₀+C₁τ) exp(-a\_R²τ) & partially retained & partial
anchoring \\
\end{longtable}
...
This loading is non-zero at the short end whenever A\_N≠0. Thus
...
policy-pinning condition is imposed at τ=0. It is not a claim that
...
short-end level. Low ρ corresponds to weak anchoring. Larger positive ρ
...
positive and lies in the relevant maturity domain. Thus ρ controls not
...
is non-negative on the half-line when C\_D≥0. It also vanishes at the
short end.

For Robin closure, sign compatibility is more delicate. In ρ-form,

\[
H_R(\tau) = C_0{[}1 + (a_R^2 - \rho)\tau{]} \exp(-a_R^2\tau)
\]

For C₀\textgreater0, non-negativity on the full half-line requires

\[
\rho \le a_R^2
\]

If ρ\textgreater a\_R², the linear factor eventually changes sign. Such
...
each fixed τ\textgreater0, and the lower boundary is inaccessible
...
\[
quadratic models generate shape through nonlinear state mapping;
\]
...
A maturity loading expressed in τ-coordinates becomes a loading in
...
The key object is the maturity loading H(τ). In the one-factor IRFR
model,
...
Thus H(τ) determines the maturity shape of volatility.
...
recovering the empirical maturity loading H(τ), and possibly its scale,
...
structure --- to structural parameters such as k, ρ, and the boundary
regime.
...
quantities. Recovering x(t,τ) and estimating changes in H(τ) therefore
...
\begin{quote}
• 1. Estimate the maturity loading, not only the volatility level.

• 2. Separate stochastic dimension from maturity geometry.

• 3. Identify the decay scale.

• 4. Estimate boundary anchoring from short-end behaviour.

\[
• 5. Do not infer r = 1 from hump resemblance alone
\]

• 6. Distinguish scale from shape.

• 7. Use principal components carefully.

• 8. Treat boundary regimes as estimable hypotheses.

• 9. Check positivity and admissibility separately.

• 10. Treat empirical fit as evidence, not proof.
\end{quote}
...
object. The structural object is the shape of H(τ): where the curve is
...
what is the estimated shape of H(τ)?
...
is encoded in H(τ), and this function may be non-trivial.
...
that function is the empirical counterpart of H(τ).
...
maturity loading H(τ). This is a testable restriction, not merely an
interpretation.
...
h\_D\textquotesingle(τ)=e\^{}\{-a²τ\}(1-a²τ).
...
H\textquotesingle(0)=-ρH(0).

\[
Thus, whenever H(0) \neq 0,
\]

ρ=-H\textquotesingle(0)/H(0).
...
A low value of ρ corresponds to weak anchoring: the loading leaves the
boundary relatively freely. A high value of ρ corresponds to stronger
suppression of the short-end direction.

For the repeated-root Robin loading,

H\_R(τ)=(C₀+C₁τ)e\^{}\{-a\_R²τ\},
...
H\_R\textquotesingle(0)=C₁-a\_R²C₀.
...
smoothing, interpolation, and the chosen parametric form. Estimates of ρ
...
Multiplying H(τ) by a constant changes the volatility scale, but not the
underlying shape after normalisation. Conversely, changing k or ρ
...
\begin{quote}
• estimate the leading maturity loading;

• normalise its scale;

• compare its shape with IRFR boundary-selected forms.
\end{quote}
...
\[
In the Dirichlet r = 1 case, the selected loading
\]

H\_D\^{}\{(1)\}(τ)=C\_Dτe\^{}\{-a\_D²τ\}

is non-negative on the half-line when C\_D ≥ 0 and vanishes at τ = 0.
...
Dirichlet branch admits a shifted inverse-gamma stationary density for
each fixed τ \textgreater{} 0. The same Feller classification as the
...
maturity coordinate τ. Under such regularity, fixed-τ Feller
...
H\_R(τ)=C₀{[}1+(a\_R²-ρ)τ{]}e\^{}\{-a\_R²τ\}.

For C₀ \textgreater{} 0, non-negativity on the full half-line requires
...
\begin{quote}
• loading-level sign compatibility;

• fixed-maturity state-domain preservation under the shifted KFE and
Feller conditions;

• field-level preservation across all maturities, which requires
maturity regularity.
\end{quote}
...
\begin{quote}
• admissibility is derived;

• minimality is argued;

• canonicality is conjectured;

• empirical relevance is tested.
\end{quote}
...
\begin{quote}
• 1. Estimate the empirical maturity loading H(τ).

• 2. Normalise the loading to separate shape from scale.

• 3. Estimate k from long-maturity decay or hump location.

\[
• 4. Estimate \rho from short-end slope where H(0) \neq 0
\]

• 5. Compare Neumann/free, Dirichlet, and Robin boundary-regime fits.

• 6. Check loading sign compatibility and, where relevant,
lower-boundary admissibility.

• 7. Test stability across time and policy regimes.
\end{quote}
...
Fifth, estimate the decay scale k, or a² under the repeated-root
convention. If a Dirichlet-type hump is present, the hump location gives
the diagnostic a²≈1/τ\_max.
...
as ρ may be time-varying, reflecting changing policy regimes rather than
...
The central empirical object is the maturity loading H(τ). It determines
...
direction is proportional to H(τ).
...
when H(0) ≠ 0, though in practice both H(0) and H′(0) may require
...
where a² is the temporal drift--diffusion scale and λ² is the
maturity-decay scale. For r ≠ 1, the admissible maturity modes are
...
Thus the maturity loading H(τ) is the observable volatility-shape
object. Boundary-selected loadings become empirical hypotheses when
their parameters are estimated from data.

The structural parameters a², r, and, in the Robin case, ρ, are
...
For τ \textgreater{} 0, the repeated-root Dirichlet loading is

Under the positive normalisation C\_D ≥ 0, this loading is non-negative
...
fixed τ \textgreater{} 0, the shifted state may be represented in the
form
...
fixed τ \textgreater{} 0, provided the process starts in the positive
...
with C₀ \textgreater{} 0, non-negativity on the full half-line requires
...
conditions on discounted asset prices. The instantaneous short rate then
plays a central role in the pricing construction. In the IRFR framework,
however, the short end is itself part of the boundary-field structure.
The boundary law is not an afterthought; it is one of the mechanisms by
which observable maturity loadings are selected.

A direct risk-neutral formulation would therefore need to confront the
relation between the money-market numeraire, the zero-return or
instantaneous-rate pricing construct, and the boundary law at

The issue is not merely to translate the model into HJM notation, but to
determine how the boundary-field interpretation interacts with
no-arbitrage pricing when the short end is itself a structural boundary.
...
\[
Neumann/free closure: short-end stochastic loading retained;
\]
\[
Dirichlet closure: short-end stochastic loading suppressed;
\]
\[
Robin closure: short-end stochastic loading partially retained
\]
...
under which fixed-τ Feller inaccessibility lifts to lower-boundary
...
numeraire-based pricing construction.
...
maturities τ \textgreater{} 0, and the lower boundary is inaccessible
...
where t is calendar time and τ is remaining maturity. This change of
...
is the fixed-τ transport correction generated by rolling maturity.
...
where a² is the temporal drift-diffusion scale and λ² is the
maturity-decay scale. For r≠1, the admissible maturity modes are the
distinct exponentials

e\^{}\{-a²τ\} and e\^{}\{-ra²τ\}.

At r=1, the two modes coalesce. The repeated-root space contains
...
Natural or free closure provides the non-pinned benchmark. It retains
...
When C\_0≠0, the same loading may be written in anchoring-ratio form as
...
no-arbitrage representation of forward-rate dynamics; IRFR explains how
...
notation. A full pricing formulation would need to address the
numeraire, the money-market account, market price of risk, discounting
conventions, and the way a boundary-field short end interacts with the
zero-return or instantaneous-rate pricing construct.
...
boundary for each fixed τ\textgreater0. The remaining technical
...
density and Feller inaccessibility for τ\textgreater0. A full
maturity-field theorem requires the corresponding regularity in τ, so
...
\]
The label ``Neumann/free'' is used in the broad boundary-value sense of a non-pinned natural endpoint regime. It should not be read as imposing \(H'(0)=0\) as the operative loading condition in the final taxonomy.
...
H_D^{(1)}(\tau)=C_D\tau e^{-a_D^2\tau},
\]
with theoretical normalisation
\[
C_D=\frac{a_D^2}{2}(x_D-x_\infty),
\]
up to the sign convention of the difference quotient.
