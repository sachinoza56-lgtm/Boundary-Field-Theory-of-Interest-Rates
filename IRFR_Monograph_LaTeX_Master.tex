...
\chapter{Bond Pricing and the Risk-Neutral IRFR Measure}
...
Dirichlet, or Robin closure. What remains is the formal bridge from
admissible maturity fields to tradable bond prices.

This chapter supplies that bridge. We identify the IRFR field with the
instantaneous forward-rate curve in rolling-maturity coordinates, define
zero-coupon bond prices by exponential discounting, introduce the
money-market numeraire, and impose the no-arbitrage martingale condition.
The resulting construction is an IRFR-constrained HJM framework: IRFR
restricts the admissible maturity geometry of the volatility modes, while
the risk-neutral measure determines the drift required for arbitrage-free
bond pricing \cite{HeathJarrowMorton1992,MusielaRutkowski2005}.

The chapter does not claim to give a complete derivative-pricing theory.
It establishes the bond-pricing layer and formulates the natural extension
to bond options and other interest-rate derivatives. Derivative pricing,
market completeness, and empirical calibration of risk premia remain
separate tasks.

\section{The IRFR Field as a Rolling Forward-Rate Curve}
\label{irfr-field-as-rolling-forward-rate-curve}

Let \(f(t,T)\) denote the instantaneous forward rate observed at calendar
time \(t\) for calendar maturity \(T\geq t\). The IRFR field is written in
rolling-maturity coordinates, where
\[
\tau = T-t.
\]
We identify the IRFR field with the rolling representation of the
instantaneous forward-rate surface:
\[
x(t,\tau)=f(t,t+\tau).
\]
Conversely, for \(T\geq t\),
\[
f(t,T)=x(t,T-t).
\]

This identification requires sufficient regularity of the forward-rate
surface for the calendar-maturity and rolling-maturity descriptions to
agree pointwise. In particular, the forward surface is assumed to be
sufficiently smooth in the maturity variable for the change of variables
\(T=t+\tau\) to be valid. Under this convention, \(x(t,\tau)\) is not merely
a generic maturity field. It is the instantaneous forward-rate curve
expressed in the natural IRFR coordinate \(\tau\).

The distinction between calendar maturity and rolling maturity matters
for dynamics. Since \(\tau=T-t\), differentiation at fixed \(T\) and
differentiation at fixed \(\tau\) differ by the transport term:
\[
\left.\partial_t f(t,T)\right|_{T}
=
...
\qquad \tau=T-t.
\]
Equivalently, the rolling-maturity drift contains the calendar-maturity
forward drift plus the transport contribution \(\partial_\tau x\). This
point is essential when the HJM no-arbitrage condition is translated into
IRFR coordinates.

\section{Zero-Coupon Bond Prices from the IRFR Field}
\label{zero-coupon-bond-prices-from-irfr}

The standard zero-coupon bond price at time \(t\) for maturity \(T\) is
\[
P(t,T)
=
\exp\left(
...
\right).
\]
Using the rolling-maturity coordinate \(s=u-t\), this becomes
\[
P(t,t+\tau)
=
\exp\left(
...
\right).
\]
This equation is the basic pricing interpretation of the IRFR field. Once
\(x(t,\tau)\) is identified with the instantaneous forward-rate curve,
admissible IRFR maturity profiles determine admissible discount curves.

The formula becomes more informative when the IRFR modal structure is
substituted. Suppose, for example, that under a given boundary regime the
forward-rate field has the one-factor representation
\[
x(t,\tau)=x_D+Z_tH(\tau),
\]
where \(x_D\) is the pinned short-end boundary value, \(Z_t\) is the
stochastic factor amplitude, and \(H(\tau)\) is an IRFR-admissible maturity
loading. Then
\[
P(t,t+\tau)
=
...
For the repeated-root humped branch, take
\[
...
\int_0^\tau H(s)\,ds
=
C\int_0^\tau s e^{-\alpha s}\,ds
=
\frac{C}{\alpha^2}
\left[
1-e^{-\alpha\tau}(1+\alpha\tau)
\right].
\]
Hence the zero-coupon bond price becomes
\[
P(t,t+\tau)
=
\exp\left(
...
\right).
\]
This expression shows explicitly how the IRFR humped admissible mode
enters bond prices through the integrated forward-rate loading.

\section{The Money-Market Numeraire and Boundary Regimes}
\label{money-market-numeraire-and-boundary-regimes}

The short rate is the zero-maturity limit of the forward-rate field:
\[
r_t=x(t,0^+).
\]
...
Under Dirichlet closure, the short end is pinned:
\[
x(t,0^+)=x_D.
\]
In that case,
...
B_t=\exp(x_Dt),
\]
conditional on the boundary regime. This is a pricing-relevant
consequence of the boundary condition: the short-rate numeraire is locally
deterministic while the stochastic variation enters through the interior
maturity loading.

The other boundary regimes have different pricing implications. Under
Neumann closure, the slope at the boundary is pinned, but the boundary
value itself may move. Under Robin closure, the boundary value and
boundary slope are linked through a mixed condition. Thus the boundary
regime determines not only the admissible maturity modes but also the way
the short-rate numeraire is generated from the field.

\section{Measure Change and the Market Price of Risk}
\label{measure-change-and-market-price-of-risk}

Let the rolling IRFR forward-rate field evolve under the physical measure
\(\mathbb P\) according to
\[
dx(t,\tau)
=
b^{\mathbb P}(t,\tau)\,dt
+
\sum_{i=1}^n \sigma_i(t,\tau)\,dW_i^{\mathbb P}(t),
\]
where the maturity profiles \(\sigma_i(t,\tau)\) are required to be
IRFR-admissible. We use \(b\) for the rolling-maturity drift in this
chapter to distinguish it from the calendar-maturity HJM drift.

Introduce a market price of risk vector
\[
\lambda_t=(\lambda_1(t),\ldots,\lambda_n(t)).
\]
The risk-neutral Brownian motions are defined by
\[
dW_i^{\mathbb Q}(t)
=
dW_i^{\mathbb P}(t)
...
\lambda_i(t)\,dt.
\]
Substitution gives the \(\mathbb Q\)-dynamics
\[
dx(t,\tau)
=
b^{\mathbb Q}(t,\tau)\,dt
+
\sum_{i=1}^n \sigma_i(t,\tau)\,dW_i^{\mathbb Q}(t),
\]
where
\[
b^{\mathbb Q}(t,\tau)
...
b^{\mathbb P}(t,\tau)
-
\sum_{i=1}^n \sigma_i(t,\tau)\lambda_i(t).
\]

This measure change is assumed to satisfy the usual integrability
conditions, such as a Novikov-type condition, ensuring that the density
process defining \(\mathbb Q\) is a true martingale. In a reduced affine
IRFR implementation, one may further assume an affine market price of risk
so that the affine closure of the physical dynamics is preserved under the
change of measure. This affine preservation is an assumption on the
pricing specification, not a consequence of admissibility alone.

\section{The Market Signal-to-Noise Ratio}
\label{market-signal-to-noise-ratio}

The market price of risk has a natural interpretation in IRFR language
through the Market Signal-to-Noise Ratio, or MSNR. In one-factor rolling
notation, suppose
\[
dx(t,\tau)
=
...
\sigma(t,\tau)\,dW_t.
\]
Define the Market Signal-to-Noise Ratio by
\[
\mathrm{MSNR}(t,\tau)
=
\frac{b(t,\tau)}{\sigma(t,\tau)}.
\]
It measures the local rolling drift signal per unit of diffusion noise in
the maturity field.

Under the physical measure,
\[
\mathrm{MSNR}^{\mathbb P}(t,\tau)
=
\frac{b^{\mathbb P}(t,\tau)}{\sigma(t,\tau)}.
\]
Under the risk-neutral measure,
\[
\mathrm{MSNR}^{\mathbb Q}(t,\tau)
=
\frac{b^{\mathbb Q}(t,\tau)}{\sigma(t,\tau)}.
\]
Since
\[
b^{\mathbb Q}(t,\tau)
...
\mathrm{MSNR}^{\mathbb Q}(t,\tau).
\]
Thus the market price of risk is the measure-change correction to the
Market Signal-to-Noise Ratio. It is the excess physical rolling drift
signal, per unit diffusion noise, that must be removed to pass from the
observed physical dynamics to the no-arbitrage risk-neutral dynamics.

In the IRFR sign convention used earlier, the local field dynamics are
often written as
\[
dx=-f\,dt+\sqrt{v}\,dW.
\]
In that convention, the rolling drift signal is \(-f\) and the noise scale
is \(\sqrt v\). The Market Signal-to-Noise Ratio is therefore
\[
\mathrm{MSNR}
=
\frac{-f}{\sqrt v}.
\]
The measure-change relation becomes
\[
\lambda
=
\frac{-f^{\mathbb P}}{\sqrt v}
-
\frac{-f^{\mathbb Q}}{\sqrt v}
=
\frac{f^{\mathbb Q}-f^{\mathbb P}}{\sqrt v}.
\]
Equivalently,
\[
f^{\mathbb Q}
...
\lambda\sqrt v.
\]
This gives \(\lambda\) a direct IRFR interpretation: it is not an external
black-box risk premium, but the adjustment to the Market Signal-to-Noise
Ratio required by the change from the physical measure to the
risk-neutral measure.

In the multi-factor case, the same interpretation holds in vector form,
but uniqueness may fail. If
\[
dx(t,\tau)
=
b(t,\tau)\,dt
...
\Sigma(t,\tau)^+
\left(
 b^{\mathbb P}(t,\tau)-b^{\mathbb Q}(t,\tau)
\right),
\]
where \(\Sigma^+\) denotes a suitable pseudo-inverse. If the volatility
loading is not full rank, the market is incomplete and the market price
of risk need not be unique.

\section{The HJM Drift Restriction in IRFR Coordinates}
\label{hjm-drift-restriction-in-irfr-coordinates}

Under the money-market numeraire, no-arbitrage requires discounted
zero-coupon bond prices
\[
\frac{P(t,T)}{B_t}
\]
to be \(\mathbb Q\)-martingales for every maturity \(T\).

In the standard HJM formulation, if
\[
df(t,T)
=
\alpha^{\mathbb Q}(t,T)\,dt
+
\sum_{i=1}^n \sigma_i(t,T)\,dW_i^{\mathbb Q}(t),
\]
where the derivative is taken at fixed calendar maturity \(T\), then the
no-arbitrage drift restriction is
\[
\alpha^{\mathbb Q}(t,T)
=
\sum_{i=1}^n
...
In rolling IRFR coordinates, \(T=t+\tau\), the calendar-maturity drift
restriction becomes
\[
\alpha^{\mathbb Q}(t,\tau)
=
\sum_{i=1}^n
\sigma_i(t,\tau)
\int_0^\tau \sigma_i(t,s)\,ds.
\]
The rolling drift is not just \(\alpha^{\mathbb Q}\). By the transport
identity, the \(\mathbb Q\)-drift of \(x(t,\tau)\) at fixed rolling maturity
is
\[
b^{\mathbb Q}(t,\tau)
=
...
\int_0^\tau \sigma_i(t,s)\,ds.
\]
Therefore the risk-neutral IRFR dynamics in rolling coordinates are
\[
dx(t,\tau)
=
\left[
...
+
\sum_{i=1}^n
\sigma_i(t,\tau)\,dW_i^{\mathbb Q}(t).
\]
This is the Musiela-form HJM equation written in IRFR notation.

If the volatility modes separate into stochastic amplitudes and IRFR
maturity loadings,
\[
\sigma_i(t,\tau)=\eta_i(t)H_i(\tau),
\]
then the HJM compensator is
\[
\alpha^{\mathbb Q}(t,\tau)
=
\sum_{i=1}^n
...
\int_0^\tau H_i(s)\,ds,
\]
and the rolling risk-neutral drift is
\[
b^{\mathbb Q}(t,\tau)
=
\partial_\tau x(t,\tau)
...
For the one-factor repeated-root humped branch,
\[
H(\tau)=C\tau e^{-\alpha\tau},
\]
we obtain
...
\alpha^{\mathbb Q}(t,\tau)
=
\frac{\eta(t)^2 C^2}{\alpha^2}
\tau e^{-\alpha\tau}
\left[
1-e^{-\alpha\tau}(1+\alpha\tau)
\right].
\]
This is the HJM no-arbitrage compensator generated by the IRFR humped
volatility loading.

The corresponding risk-neutral Market Signal-to-Noise Ratio for the
rolling field is
\[
\mathrm{MSNR}^{\mathbb Q}(t,\tau)
=
\frac{b^{\mathbb Q}(t,\tau)}{\sigma(t,\tau)}
=
\frac{\partial_\tau x(t,\tau)}{\sigma(t,\tau)}
+
\int_0^\tau \sigma(t,s)\,ds
\]
in the one-factor case. Thus
\[
\lambda_t
=
\mathrm{MSNR}^{\mathbb P}(t,\tau)
...
\right].
\]
This formula connects the empirical signal-to-noise structure of the
physical IRFR field with the transport-corrected no-arbitrage compensator
required under the pricing measure.

\section{Reconciling IRFR Admissibility with the HJM Drift Restriction}
\label{reconciling-irfr-admissibility-with-hjm}

IRFR admissibility and the HJM no-arbitrage condition are not the same
restriction. They act on different components of the model.

IRFR admissibility restricts the allowable maturity loadings:
\[
H_i(\cdot)\in\mathcal A_{\mathrm{IRFR}},
\]
where \(\mathcal A_{\mathrm{IRFR}}\) denotes the space of maturity profiles
compatible with the field equation and the imposed boundary regime. The
HJM condition does not choose these loadings. Instead, once the loadings
have been chosen, it determines the drift required to make discounted
bond prices martingales.

Thus the appropriate structure is
\[
\sigma_i(t,\tau)=\eta_i(t)H_i(\tau),
\qquad
H_i\in\mathcal A_{\mathrm{IRFR}},
\]
together with
\[
b^{\mathbb Q}(t,\tau)
...
\int_0^\tau \sigma_i(t,s)\,ds.
\]
The HJM drift need not itself be a homogeneous IRFR admissible mode. It is
the no-arbitrage compensator associated with the admissible volatility
geometry, plus the transport contribution required by rolling-maturity
coordinates.

Consequently, IRFR admissibility does not automatically imply
arbitrage-free pricing. Rather, IRFR admissibility and no-arbitrage are
compatible when admissibility is imposed on the volatility maturity modes
and the risk-neutral drift is supplied by the HJM restriction. The
resulting model is an IRFR-constrained HJM model: its volatility geometry
is field-admissible, and its pricing dynamics are martingale-consistent.

The Market Signal-to-Noise Ratio clarifies this division of labour. Under
\(\mathbb P\), MSNR measures the empirical rolling drift signal per unit
diffusion noise. Under \(\mathbb Q\), the drift signal is replaced by the
transport-corrected HJM drift. The market price of risk \(\lambda\) is the
difference between these two signal-to-noise ratios. Thus \(\lambda\) is
the object that reconciles physical IRFR dynamics with risk-neutral bond
pricing.

\section{Toward Derivative Pricing}
\label{toward-derivative-pricing}

The construction above establishes the risk-neutral bond-pricing layer of
IRFR. Once zero-coupon bond prices are defined by
\[
P(t,T)
=
\exp\left(
...
\right),
\]
and once the forward-rate field is placed under the risk-neutral measure
\(\mathbb Q\), contingent claims written on bonds can in principle be
priced by the usual martingale valuation formula.

For a payoff \(\Phi\) at maturity \(S\), the time-\(t\) value under the
money-market numeraire is
\[
V(t)
=
B_t\,\mathbb E_t^{\mathbb Q}
\left[
B_S^{-1}\Phi
\right].
\]
For example, a European option on a zero-coupon bond maturing at
\(T>S\) has payoff
\[
\Phi=(P(S,T)-K)^+.
\]
...
\right].
\]
Equivalently, using the \(S\)-maturity bond as numeraire, the same option
can be written as
\[
C(t)
=
P(t,S)\,\mathbb E_t^{\mathbb Q^S}
\left[
(P(S,T)-K)^+
\right],
\]
where \(\mathbb Q^S\) is the \(S\)-forward measure.

In IRFR terms, the underlying bond price is a functional of the field:
\[
P(S,T)
=
\exp\left(
...
\right).
\]
Thus a bond option is a contingent claim on an integrated IRFR field. The
admissible maturity loading \(H(\tau)\) determines how shocks to the field
propagate into the bond price. If, for example,
\[
x(t,\tau)=x_D+Z_tH(\tau),
\]
then
...
\right).
\]
In this reduced one-factor representation, bond-option pricing reduces to
the pricing of a contingent claim on the scalar factor \(Z_t\), with bond
sensitivity determined by the integrated IRFR loading
\[
\int_0^\tau H(s)\,ds.
\]

For the repeated-root humped branch,
\[
H(\tau)=C\tau e^{-\alpha\tau},
\]
...
\right].
\]
Hence the field-to-bond mapping is explicit. This is the natural point at
which an IRFR derivative-pricing theory would begin: the admissible
maturity mode determines the bond-price exposure, while the risk-neutral
dynamics determine the distribution of that exposure under \(\mathbb Q\).

However, this chapter does not complete that derivative-pricing
programme. A full theory would require specifying the risk-neutral
dynamics of the reduced factor process, proving the relevant
distributional results, and deriving or numerically approximating option
prices for bond options, caps, floors, and swaptions. In particular, the
extension from bond pricing to option pricing requires more than the
existence of a risk-neutral measure. It requires a tractable
representation of the law of the integrated IRFR field under \(\mathbb Q\).

The relation to humped-volatility derivative models should therefore be
stated carefully. IRFR supplies a structural mechanism by which humped
maturity loadings arise from admissibility and boundary selection. It
does not, at this stage, reproduce a full option-pricing framework such
as Ritchken--Chuang \cite{RitchkenChuang1999}. The appropriate claim is
narrower: IRFR identifies an endogenous field-theoretic route to
maturity-dependent volatility loadings that can, in principle, be inserted
into a derivative-pricing framework once the risk-neutral factor dynamics
are fully specified.
...
\begin{enumerate}
\item the IRFR field can be identified with the instantaneous forward-rate
curve in rolling-maturity coordinates;
\item zero-coupon bond prices can be defined by integrating the IRFR field;
\item the short rate and money-market numeraire are determined by the
short-end boundary value;
\item a risk-neutral measure can be introduced through a standard
market-price-of-risk adjustment;
\item the market price of risk can be interpreted as the correction
between physical and risk-neutral Market Signal-to-Noise Ratios;
\item the HJM drift restriction can be written in transport-corrected IRFR
coordinates;
\item IRFR-admissible volatility modes can be made consistent with
no-arbitrage bond pricing;
\item the derivative-pricing extension can be formulated as pricing
functionals of the integrated IRFR field under \(\mathbb Q\).
\end{enumerate}

The chapter does not establish a complete derivative-pricing theory. In
particular, it does not prove uniqueness of the risk-neutral measure,
market completeness, or closed-form pricing for options on bonds, caps,
floors, or swaptions. Nor does it prove empirical superiority over affine
or HJM term-structure models. Those questions require additional
assumptions about the market price of risk, the rank of the volatility
loading structure, and the calibration of the resulting model to market
data.

The construction also does not imply that IRFR admissibility alone
enforces no-arbitrage. The field equation and boundary regime restrict
the admissible maturity geometry. The martingale pricing condition
supplies the risk-neutral drift. These are complementary restrictions,
not identical ones.

The main conclusion is therefore precise: IRFR can be given a
no-arbitrage bond-pricing interpretation by identifying the field with the
instantaneous forward-rate curve and imposing an HJM-style drift
restriction on IRFR-admissible volatility modes. The open problem is no
longer the conceptual existence of a risk-neutral bond-pricing measure.
What remains open is the empirical calibration, numerical implementation,
and derivative-pricing extension of the resulting IRFR-constrained HJM
framework.
